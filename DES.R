...
#the job's arrival time in queuing app

#a global list named "sim" holdsthe events data frame, evnts and current
#simulated time, currtime; there is also a component dbg, which indicates 
#debugging mode.
...
evntrow <- function(evnttm,evntty,appin=NULL){
  rw <- c(list(evnttim=evnttm,evnttype=evntty),appin)
...
#length(x) + 1 if y is larger than x[length(x)]; could be changed to C code for efficiency
...
#start to process next event(second half done by application programmer bia call to reactevnt())
...
#Simulate body
#arguments:
# initglbls: application-specific initialization function; inits globals to statistical totals for the app, etc.;
#            records appears in globals; schedule firt event
# reactevnt: application-specific event handling function, coding the proper action for each type of event
# prntrslts: prints application-specific results, e.g. mean queue wait
# apppars:   list of application-specific parameterws, e.g. number of servers in a queuing app
# maxsimtime: simulation will be run until this simulation time
...
dosim <- function(initglbls,reactevnt,prntrslts,maxsimtime,apppars=NULL, dbg=FALSE) {
  ...
  sim$dbg <<- dbg
  initglbls(apppars)
  while(sim$currtime < sim$maxtime){
    head <- getnextevnt()
